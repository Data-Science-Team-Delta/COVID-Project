...
theme_common <- function() {
  ...
  theme(
    ...
    legend.title = element_text(margin = margin(4, 4, 4, 4), size = 16),
...
df_data %>%
  ...
  ungroup() %>%
  drop_na(num_measures_perday) %>%
  mutate(bin = cut(avg_delta_cases, seq(0, max(avg_delta_cases), length.out = 100), labels = FALSE)) %>%
  mutate(bin = bin * max(avg_delta_cases) / 100) %>%
  ggplot() +
  geom_histogram(
  ...
  geom_smooth(
    ...
    mapping = aes(bin, num_measures_perbin),
    color = "Black",
    se = FALSE
  ) +
  xlab("Average change in state covid cases (normalized for population) over the 14 days before each measure") +
  ylab("US government Covid-19 intervention measures") +
  ggtitle("Number of US intervention measures vs average change in state covid cases before measure") +
  theme_common()
```
...
df_data %>%
  ...
  mutate(bin = cut(avg_cases, seq(0, max(avg_cases), length.out = 100), labels = FALSE)) %>%
  mutate(bin = bin * max(avg_cases) / 100) %>%
  ggplot() +
  geom_histogram(
  ...
  ylim(0, NA) +
  xlab("Average state covid cases (normalized for population) over the 14 days before each measure") +
  ylab("US government Covid-19 intervention measures") +
  ggtitle("Number of US intervention measures vs average state covid cases before measure") +
  theme_common()
```
